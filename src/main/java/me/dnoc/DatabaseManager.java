...
public class DatabaseManager implements AutoCloseable {
    ...
    public Connection getConnection() throws SQLException {
        try {
            ...
            LOGGER.severe(String.format("Failed to get database connection: %s", e.getMessage()));
            throw e;
        }
    }
    ...
    public void setColumnWhitelist(Set<String> columnWhitelist) {
        ...
        LOGGER.info(String.format("Column whitelist updated with %d entries", this.columnWhitelist.size()));
    }
        ...
        try (Connection connection = getConnection()) {
            for (String column : placeholders.keySet()) {
                if (!isValidColumn(column)) {
                    LOGGER.warning(String.format("Skipping invalid column name: %s", column));
                    continue;
                }
                ...
                if (!columnExists(connection, column)) {
                    ...
                    try {
                        ...
                        LOGGER.info(String.format("Added new column: %s", column));
                    } catch (SQLException e) {
                        LOGGER.severe(String.format("Failed to add column %s: %s", column, e.getMessage()));
                    }
                }
            }
        } catch (SQLException e) {
            LOGGER.severe(String.format("Database error while ensuring columns: %s", e.getMessage()));
        }
    }
    ...
    public void ensureTableExists() {
        try {
            ...
            LOGGER.severe(String.format("Failed to ensure table exists: %s", e.getMessage()));
        }
    }

    private boolean tableExists() throws SQLException {
        try (Connection connection = getConnection(); ResultSet rs = connection.getMetaData().getTables(null, null, "PlayerData", new String[]{"TABLE"})) {
            return rs.next();
        }
    }
    ...
    public void updatePlayerOnlineStatus(String username, boolean isOnline) throws SQLException {
        String query = "UPDATE PlayerData SET online = ? WHERE username = ?";
        executeUpdate(query, isOnline, username);
    }
}
